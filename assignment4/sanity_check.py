...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    assert np.allclose(
        dec_init_state_target[0].numpy(), dec_init_state_pred[0].numpy()
    ...
    assert np.allclose(
        dec_init_state_target[1].numpy(), dec_init_state_pred[1].numpy()
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    assert np.allclose(
        combined_outputs_pred.numpy(), combined_outputs_target.numpy()
