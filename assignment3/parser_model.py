...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # self.pretrained_embeddings = nn.Embedding.from_pretrained(torch.tensor(embeddings, dtype=torch.float))

        # Linear layer from concatenated embeddings to hidden layer
        ...
        # Dropout later fir regularization
        self.dropout_prob = nn.Dropout(dropout_prob)

        # Linear layer from hidden layer to output logits
    ...
    def embedding_lookup(self, t):
        ...
        embedded = self.pretrained_embeddings(t)

        batch_size = t.size(0)
        x = embedded.view(batch_size, -1)
    ...
    def forward(self, t):
        ...
        # Step 1: Get embeddings from tokens
        embeddings = self.embedding_lookup(t)

        # Step 2: Apply the linear layer to the transform embeddings to hidden layer representation
        hidden = self.embed_to_hidden(embeddings)

        # Step 3: Apply ReLU non-linearity to hidden layer output
        hidden = F.relu(hidden)

        # Step 4: Apply dropout for regularization
        hidden = self.dropout_prob(hidden)

        # Step 5: Transform the output of the dropout layer to logits
